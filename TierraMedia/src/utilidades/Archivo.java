...
public class Archivo {
	...
	public List<Promocion> LeerPromociones(String archivo, List<Atraccion> listaAtracciones) throws Exception {
		...
		try {
			while ((linea = bufferReader.readLine()) != null) {
				...
				int porcentaje = datos[4] != null ? Integer.parseInt(datos[4]) : 0;
				
				listaPromociones.add(crearPromocion(datos[0],atracciones, datos[2], datos[3], porcentaje));
			}
	...
	private Promocion crearPromocion(String nombre, List<Atraccion> atracciones, String tipo, String descripcion, int porcentaje) {
		
			switch (tipo) {
				case "Porcentual":
						return new Porcentual(nombre, atracciones, descripcion, porcentaje);
					default: 
						return null;
				}
		}
